...
for loc in range(0,100):
    ...
    chunks = pd.read_csv('genotype_matrix.txt', delimiter='\t', header=None, chunksize=chunk_size)

    # Calculate the chunk range that needs to be processed
    start_chunk = start_idx // chunk_size
    end_chunk = (end_idx + chunk_size - 1) // chunk_size

    for i, chunk in enumerate(chunks):
        if i < start_chunk:
            continue  # Skip chunks before the start_chunk
        if i > end_chunk:
            break  # Stop processing if past the end_chunk
        
        # Determine the actual row indices in the current chunk
        chunk_start = max(0, start_idx - i * chunk_size)
        chunk_end = min(chunk.shape[0], end_idx - i * chunk_size)
        
        if chunk_start < chunk_end:
            df_subset = chunk.iloc[chunk_start:chunk_end]
    ...
    output_file = f"/no_maf_regular_coverage/haplotype_fraction_starting_location_{starting_locations[loc]}.csv"

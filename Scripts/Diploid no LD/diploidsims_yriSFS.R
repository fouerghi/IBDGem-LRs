...
getsims <- function(n, k, eps, nsim,number_of_reads,snp_matrix,order_number,target_genotype_k,reads,loci_frequency){
  ...
  # Read in the sfs we'll be using to generate genotypes
  yri_sfs <- read.table("~/Desktop/IBDGem Followup/yri_sfs.txt", header = FALSE, col.names = "value")$value
...
# generating genotype(s) that obey(s) allele frequencies derived from YRI SFS

#generate n.loci allele frequencies from YRI SFS that
...
# read in the allele frequencies derived from YRI subpopulation in 1kGP
yri_sfs <- read.table("yri_sfs.txt", header = FALSE, col.names = "value")$value

#function to generate the target individual's genotype according to YRI SFS
generate_genotype <- function(k,yri_sfs) {
  if (k == 1) {
    ...
    ps <- gen.realistic.sfs(k, yri_sfs,length(yri_sfs))
    ref.test <- sim.ref.varfreq.diploid(1, ps)
    return(list(ref.test=ref.test,ps=ps))
  }
}

# function to generate the reference panel genotypes obeying allele frequencies in ps
# ps is what was used to generate the target individual's genotype
...
result <- generate_genotype(k=500,yri_sfs)
...
plot_results <- function(xvals, df, xlab, ylab, main,xlim=NULL) {
  ...
  for(i in 1:length(nvals)){
    points(rep(xvals[i], nsim), df[101:nrow(df),i], col = "black", pch = 19)
  }
...
pdf("FigS1.pdf", width = 8, height = 6)

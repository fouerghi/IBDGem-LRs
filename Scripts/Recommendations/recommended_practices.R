...
sampled_indices <- sample(1:nrow(target_genotype_500_loci), 50, replace=FALSE)
target_genotype_50_loci <- target_genotype_500_loci[sampled_indices, ]
#indices we extracted -- now ordered and unlisted
indices <- sort(unlist(sampled_indices,use.names=FALSE))
snp_matrix_sampled_50_loci <- snp_matrix_500_loci[indices,]
...
## Sample indices uniformly at random to form the target genotype's at k=5 loci
sampled_indices <- sample(1:nrow(target_genotype_50_loci), 5, replace=FALSE)
target_genotype_5_loci <- target_genotype_50_loci[sampled_indices, ]
#indices we extracted -- now ordered and unlisted
indices <- sort(unlist(sampled_indices,use.names=FALSE))
snp_matrix_sampled_5_loci <- snp_matrix_sampled_50_loci[indices,]
# generate the reads for the target genotype when k = 5
reads_5 <- simulate_reads_for_target(target_genotype_5_loci,eps=0.02,number_of_reads=2,k=5)
loci_frequency_5 <- loci_frequency_50[indices]
...
################## Vary the reference panel size according to nvals while k is set to 5 with number of reads = 2
...
for(i in 1:length(nvals)){
  n <- nvals[i]
  getsims(n, k = 5, eps = 0.02, nsim = nsim,number_of_reads=2,snp_matrix_sampled_5_loci,order_number=i,target_genotype_5_loci,reads = reads_5,loci_frequency_5)
}

##################  Vary the number of loci according to kvals while n is set to 100, eps=0.02, number of reads = 2

loci_to_sample <- kvals
...
for (num_loci in loci_to_sample) {
  ...
  target_genotypes[[paste0("target_genotype_", num_loci, "_loci")]] <- target_genotype_50_loci[1:num_loci, ]
  
  indices <- sort(unlist(sampled_indices,use.names=FALSE))
  # Extract corresponding snp matrix
  snp_matrices[[paste0("snp_matrix_sampled_", num_loci, "_loci")]] <- snp_matrix_sampled_50_loci[1:num_loci,]
  
  # Extract corresponding allele frequencies for selected subsetted loci
  loci_freq_matrices[[paste0("loci_frequency_", num_loci, "_loci")]] <- loci_frequency_50[1:num_loci]
  
  reads_list[[as.character(num_loci)]] <- reads_50[1:num_loci,]
}
...
generate_main_directory <- function(list_name, list_element, base_directory, i,type) {
  if (type == "e") {
    main_directories <- file.path(base_directory, paste0("n100_k5_r2_order", i))
  } else if (type == "k") {
    main_directories <- file.path(base_directory, paste0("n100_k", list_element, "_r2_order", i))
  } else if (type == "n") {
    main_directories <- file.path(base_directory, paste0("n", list_element, "_k5_r2_order", i))
  } else if (type == "r") {
    main_directories <- file.path(base_directory, paste0("n100_k5_r", list_element, "_order", i))
  }
...
par(mfrow = c(2,1), mar = c(4.1, 4.1, 1.1, 1.1), mgp = c(2.2, 0.8, 0))
...
plot_results(kvals, combined_df,
             "number of loci", "log likelihood ratio (base 10)", "B",xlim= range(kvals))

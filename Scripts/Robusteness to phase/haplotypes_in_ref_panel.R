#10/15/24
#Code to simulate and analyze the difference between the target individual's haplotypes being in the same reference individual
# and the two haplotypes being in two different reference individuals
...
getsims <- function(n, k, eps,nsim, number_of_reads,snp_matrix,order_number,target_genotype_k,reads,loci_frequency){
  ...
  background_list <- paste0("sample",1:(n-1))
  sample_list <- paste0("sample",n)
  for (m in 1:nsim){
    # create appropriate directories
    new_sub_directory_name <- paste0("n", n, "_k", k,"_sim",m,"_sameperson")
    ...
    genotypes_unchanged <- cbind(do.call(cbind, lapply(1:(n-1), function(x) generate_genotype_panel(k,loci_frequency)$ref.test)),target_genotype_k)
    
    genotypes_changed_1 <- genotypes_unchanged
    genotypes_changed_1[,2*n-3] <- genotypes_unchanged[,2*n-1]
    genotypes_changed_1[,2*n-2] <- genotypes_unchanged[,2*n]
    
    
    #save the .hap file
    write.table(genotypes_changed_1, file = "test.hap", sep = " ", quote = FALSE, row.names = FALSE, col.names = FALSE)
    ...
    #save background individuals
    writeLines(background_list, "background.txt")
    
    #save the sample list
    writeLines(sample_list,"samplelist.txt")
    ...
    # create appropriate directories
    other_sub_directory_name <- paste0("n", n, "_k", k,"_sim",m,"_differentperson")
    ...
    genotypes_changed_2 <- genotypes_unchanged
    genotypes_changed_2[,2*n-2] <- genotypes_unchanged[,2*n]
    genotypes_changed_2[,2*n-5] <- genotypes_unchanged[,2*n-1]
    write.table(genotypes_changed_2, file = "test.hap", sep = " ", quote = FALSE, row.names = FALSE, col.names = FALSE)
...
run_summary <- function(base_directory,xvals, type){
  ...
  for (x in xvals){
    ...
    result_df <- data.frame( log10LR_LD_differentperson = numeric(nsim),log10LR_LD_sameperson = numeric(nsim))
    index = 1
...
compress_and_modify <- function(df) {
  ...
  df_compressed$log10LR_LD_differentperson[101:nrow(df_compressed)] <- df_compressed$log10LR_LD_sameperson[101:nrow(df_compressed)]
  df_compressed <- df_compressed[, -2, drop = FALSE]
  return(df_compressed)
}

plot_results <- function(xvals, df, xlab, ylab, main,xlim=NULL,nsim) {
  ...
  for(i in 1:length(nvals)){
    points(rep(xvals[i], nsim), df[101:200,i], col = "#7570b3", pch = 1)
  }
  lines(xvals, log10(colMeans(10^df[1:nsim,])), col = "#7570b3", lty = 2  )
  lines(xvals, colMeans(df[1:nsim,]), col = "#7570b3", lty = 1  )
  
}

################## When we vary the panel size
base_directory <- "haplos_in_ref"
...
plot_results(nvals, combined_df,
             "reference panel size", "log likelihood ratio (base 10)", "B",xlim= range(c(0,nvals)),nsim=100)
legend("topleft", pch = c(19,1), col = c("#7570b3", "#7570b3"), lty = c(0,0), legend = c("IBDGem LR: haplotypes matching reads in different reference individuals", "IBDGem LR: haplotypes matching reads in same reference individuals"), bty = "n",cex=0.9)
